...
function checkProfExists($prof_name)
{
    $conn = getDatabaseConnection();

    $stmt = $conn->prepare("SELECT COUNT(*) FROM professors WHERE prof_name = :prof_name");
    $stmt->bindParam(':prof_name', $prof_name);
    $stmt->execute();
...
function checkRoomExists($room_name)
{
    $conn = getDatabaseConnection();

    $stmt = $conn->prepare("SELECT COUNT(*) FROM rooms WHERE room_name = :room_name");
    $stmt->bindParam(':room_name', $room_name);
    $stmt->execute();
    return $stmt->fetchColumn() > 0;
}
